...
class PerviewLessonControllerTest extends BaseCase
{
    public function test_visit_perview_lesson_datas_page()
    {

        $response = $this->json('GET', '/api/item/lesson/' . $this->lesson->id . '/preview');
    ...
    public function test_visit_del_lesson_datas_page()
    {
        ...
        $response = $this->actingAs($this->guest, 'mobile_api')
            ->json('GET', '/api/item/lesson/' . $this->del_lesson->id . '/preview');
